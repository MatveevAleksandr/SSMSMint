...
namespace SSMSMint.SSMS2020.Commands;

public class CopyAllHeadersCommand
{
    /// <summary>
    /// Command ID.
    /// </summary>
    public const int CommandId = 0x105;
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="CopyAllHeadersCommand"/> class.
    /// Adds our command handlers for menu (commands must exist in the command table file)
    /// </summary>
    /// <param name="package">Owner package, not null.</param>
    /// <param name="commandService">Command service to add command to, not null.</param>
    private CopyAllHeadersCommand(AsyncPackage package, OleMenuCommandService commandService, CopyHeadersFeature feature, IWorkspaceManager workspaceManager, IUINotificationManager uiNotificationManager)
    {
        ...
        logger.Info($"{nameof(CopyAllHeadersCommand)} Initialized");
    }

    /// <summary>
    /// Gets the instance of the command.
    /// </summary>
    public static CopyAllHeadersCommand Instance
    {
        get;
        private set;
    }
    ...
    {
        // Switch to the main thread - the call to AddCommand in CopyAllHeadersCommand's constructor requires
        // the UI thread.
        await ThreadHelper.JoinableTaskFactory.SwitchToMainThreadAsync(package.DisposalToken);

        OleMenuCommandService commandService = await package.GetServiceAsync(typeof(IMenuCommandService)) as OleMenuCommandService;
        Instance = new CopyAllHeadersCommand(package, commandService, feature, workspaceManager, uiNotificationManager);
    }
    ...
    private async void Execute(object sender, EventArgs e)
    {
        try
        {
            ...
            feature.CopyHeadersToClipboard(grManager, false);
        }
        catch (Exception ex)
        {
            logger.Error(ex);
