...
using SSMSMint.SSMS2020.Implementations;
...
public sealed class LocateInObjectExplorerCommand
{
    ...
    private readonly LocateInObjectExplorerFeature feature;
    ...
    private LocateInObjectExplorerCommand(
        AsyncPackage package,
        OleMenuCommandService commandService,
        IUINotificationManager uiNotificationManager,
        ISettingsManager settingsManager,
        LocateInObjectExplorerFeature feature
        )
    {
        ...
        logger.Info($"{nameof(LocateInObjectExplorerCommand)} Initialized");
    }
    ...
    /// </summary>
    /// <param name="package">Owner package, not null.</param>
    public static async Task InitializeAsync(AsyncPackage package, IUINotificationManager uiNotificationManager, ISettingsManager settingsManager, LocateInObjectExplorerFeature feature)
    {
        ...
        Instance = new LocateInObjectExplorerCommand(package, commandService, uiNotificationManager, settingsManager, feature);
    }
    ...
    private async void Execute(object sender, EventArgs e)
    {
        try
        {
            ...
            await feature.Process(tdManager);
        }
        catch (Exception ex)
        {
            logger.Error(ex);
            uiNotificationManager.ShowError("Locating in object explorer", ex.Message);
        }
    }
}
